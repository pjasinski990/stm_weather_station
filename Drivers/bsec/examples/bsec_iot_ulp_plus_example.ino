...
void ICACHE_RAM_ATTR ulp_plus_button_press()
{
    ...
    if (status == BSEC_OK)
    {
        Serial.println("ULP plus triggered successfully.");    
    } 
    else
    {
        Serial.println("ULP plus request rejected. ");
        switch (status)
        {
            case BSEC_W_SC_MODEXCEEDULPTIMELIMIT:
                Serial.println("Request came within 60 s of a previous measurement.");
                break;
            case BSEC_W_SC_MODINSUFFICIENTWAITTIME:
                Serial.println("Request came within 60 s of a ULP plus.");
                break;
...
void setup()
{
    ...
    String file_header = "\nTime(ms), IAQ, IAQ accuracy, Static IAQ, Raw_Temp(degC), Raw_Humidity(%rH), Comp_Temp(degC),  Comp_Humidity(%rH), Pressure(Pa), Gas Resistance(ohms), Gas percentage, CO2, bVOC, Stabilization status, Run in status, Bsec status";
