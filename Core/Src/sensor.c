...
BME68X_INTF_RET_TYPE app_spi_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    HAL_StatusTypeDef res = HAL_SPI_Transmit(app_spi, &command, 1, SPI_TIMEOUT);
...
void sensor_init()
{
  ...
  spi.Init.NSSPMode = SPI_NSS_PULSE_ENABLE;
    ...
    sensor_dev.read = app_spi_read; // SensorAPI_SPIx_Read
    sensor_dev.write =  app_spi_write; // SensorAPI_SPIx_Write
    ...
    //     uint8_t data = 0xa;
    //     uint8_t ret = 0;
    //     uint8_t res = (uint8_t)HAL_SPI_Transmit(&spi, &data, 1, SPI_TIMEOUT);
    //     res = (uint8_t)HAL_SPI_Receive(&spi, &ret, 1, SPI_TIMEOUT);
    //     log_write("res: %d", res);
    //     HAL_GPIO_WritePin(SENSOR_CS_PIN_GPIO_Port, SENSOR_CS_PIN_Pin, GPIO_PIN_SET);
    //     HAL_Delay(100);
    // }
